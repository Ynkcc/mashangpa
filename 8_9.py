...
def solve_problem(page: Page, problem_id: str):
    ...
    for page_num in range(2, total_pages + 1):
        ...
        # 【最终修改】使用一个函数(lambda)作为谓词，来精确匹配POST请求。
        # 这个谓词会检查URL、请求方法和POST Body中的page字段。
        with page.expect_response(
            lambda response: (
                # 1. 检查URL是否为正确的数据接口
                f"/api/problem-detail/{problem_id}/data/" in response.url and
                # 2. 检查请求方法是否为 POST
                response.request.method == "POST" and
                # 3. 检查POST的JSON数据中 'page' 字段是否为我们期望的页码
                response.request.post_data_json.get("page") == page_num
            ),
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1].isdigit():
        ...
        problem_id = input("未提供命令行参数，请输入题目ID (例如  8): ")
